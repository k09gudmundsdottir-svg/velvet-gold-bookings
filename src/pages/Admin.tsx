...
const Admin = () => {
  ...
  if (!session) {
    return <main className="min-h-screen bg-background px-5 py-20 text-foreground"><section className="mx-auto max-w-md border border-primary/25 bg-card p-8 shadow-elegant"><Shield className="mb-5 size-10 text-primary" /><p className="section-kicker">Admin access</p><h1 className="font-display text-5xl">[DEIN SALON NAME] command room.</h1><form onSubmit={submitLogin} className="mt-8 grid gap-4"><Input type="email" placeholder="Admin email" value={login.email} onChange={(event) => setLogin({ ...login, email: event.target.value })} /><Input type="password" placeholder="Password" value={login.password} onChange={(event) => setLogin({ ...login, password: event.target.value })} /><Button variant="hero" type="submit">Sign in</Button><Button variant="glass" type="button" onClick={() => lovable.auth.signInWithOAuth("google", { redirect_uri: window.location.origin + "/admin" })}>Continue with Google</Button></form></section></main>;
  }
